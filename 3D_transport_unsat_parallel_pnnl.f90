...
!!! this code use \partial(sc)/\partial(t)=-km0*(Ka*sc-lc)*A, pay attention to the value of Ka
...
!!! for 
  Double precision,Dimension(:,:,:),Allocatable::ne,tmp_ne,Se,S,Sm,theta,tmp_theta,theta_th,psi,epsi,tmp_epsi,epsi_th
  Double precision,Dimension(:,:,:),Allocatable::u,v,w,c_o,c,c0,lc_o,lc,lcc,sc_o,sc,b_o,b,ba,qc,qo,qd,k_m,alpha_c
  ...
  Double Precision::tmo,tfld,tfgd,tmd,psi_opt,psi_th,alpha2
  Double Precision::Ka1,Ka2,KH,R_igl,T_abs,PH,Khd,KPH,w1
...
!!! for input and output
  ...
  Double precision::dx,dy,dz,dt,Dlc,Db,Dld,Dgd,Dlo,Dgo,qc_0,qo_0,qd_0,Kc,Ko,invKho,Dk,Yd
  Double precision::lambda1,tau1,m1,mu,mmm_l,nnn_l,mmm_g,nnn_g,Kho,Ka,rhos,a_th,CF,OM,a_ep,Kp,w_os,a1,b1,SA
...
!!! memory issue
  ...
  Allocate (ne(Nx,Ny,Nz),Se(Nx,Ny,Nz),S(Nx,Ny,Nz),Sm(Nx,Ny,Nz),theta(Nx,Ny,Nz),theta_th(Nx,Ny,Nz),epsi(Nx,Ny,Nz),epsi_th(Nx,Ny,Nz))
  Allocate (psi(Nx,Ny,Nz),ba(Nx,Ny,Nz),qc(Nx,Ny,Nz),qo(Nx,Ny,Nz),qd(Nx,Ny,Nz),k_m(Nx,Ny,Nz),alpha_c(Nx,Ny,Nz))
     ...
     do j=1,Ny
        do k=1,Nz  
           if(.not.wall(i,j,k)) then
              ...
              theta_th(i,j,k) = min(a_th*(1.-ne(i,j,k))*rhos/1000*(CF/2.7+OM/1.0),theta(i,j,k)) !!avoid negative
!              Sm(i,j,k) = theta_th(i,j,k)/ne(i,j,k)
!              Se(i,j,k) = (S(i,j,k)-Sm(i,j,k))/(1-Sm(i,j,k))
!              theta_th(i,j,k) = ne(i,j,k)*Sm(i,j,k)
              epsi(i,j,k) = ne(i,j,k)*(1.0-S(i,j,k))
              epsi_th(i,j,k) = a_ep*ne(i,j,k)  !!can be reset
...
!!! consider effects of saturation on reaction rate
!!! need to recalculate for se heterogeinity

!!$  psi_opt = -3.  !KPa
!!$  psi_th = -15800
!!$  alpha2 = 1.47
!!$
!!$  psi = -9.8/lambda1*(Se(nxx,nyy,nzz)**(-1./m1)-1.)**(1./tau1)
!!$  if (psi.lt.psi_th) then
!!$     ba = 0.
!!$  else if (psi.gt.psi_opt) then
!!$     ba = 1.0
!!$  else
!!$     ba = 1.-(log(psi/psi_opt)/log(psi_th/psi_opt))**alpha2
!!$  endif
...
!!$           Do i=1,Nx
!!$              if(.not.wall(i,j,k)) then
!!$                 psi(i,j,k) = -9.8/lambda1*(Se(i,j,k)**(-1./m1)-1.)**(1./tau1)
!!$                 qc(i,j,k) = (-psi(i,j,k))**w1/((-psi(i,j,k))**w1+0.4**w1)*(0.1**w1+(-psi(i,j,k))**w1)/0.1**w1 * qc(i,j,k)
!!$                 qo(i,j,k) = (-psi(i,j,k))**w1/((-psi(i,j,k))**w1+0.4**w1)*(0.1**w1+(-psi(i,j,k))**w1)/0.1**w1 * qo(i,j,k)
!!$                 qd(i,j,k) = (-psi(i,j,k))**w1/((-psi(i,j,k))**w1+0.4**w1)*(0.1**w1+(-psi(i,j,k))**w1)/0.1**w1 * qd(i,j,k)
!!$              endif
!!$           Enddo
!!$        Enddo
!!$     Enddo
...
!! consider effect of moisture on bacteria dormancy

!!$  w1 = 4
!!$  psi = -9.8*1000/lambda1*(Se(nxx,nyy,nzz)**(-1./m1)-1.)**(1./tau1)/1000000  !!unit: MPa
!!$  ba = 1. - (-psi)**w1/((-psi)**w1+0.4**w1)  !!from (Manzoni,2014)
!!$
!!$  qc = ba*qc
!!$  qo = ba*qo
!!$  qd = ba*qd
...
!! change mass transfer rate with water content

  do i=1,Nx
     do j=1,Ny
        do k=1,Nz  
           if(.not.wall(i,j,k)) then
              psi(i,j,k) = -0.55*S(i,j,k)**(-3.58)/1000   !!unit: MPa
              ba(i,j,k) = 1. - (-psi(i,j,k))**w_os/((-psi(i,j,k))**w_os+Kp**w_os)  !!from (Manzoni,2014)
              qc(i,j,k) = ba(i,j,k)*qc_0
              qo(i,j,k) = ba(i,j,k)*qo_0
              qd(i,j,k) = ba(i,j,k)*qd_0
           endif
        enddo
     enddo
  enddo
...
!notde
  ...
  print*,'osmotd stress coeff =',ba(nxx,nyy,nzz)
     ...
     do j=1,Ny
        do k=1,Nz   
           if(wall(i,j,k)) then
              ...
              Dlc_eff(i,j,k) = (ne(i,j,k)-theta_th(i,j,k))**mmm_l * &
                   ((theta(i,j,k)-theta_th(i,j,k))/(ne(i,j,k)-theta_th(i,j,k)))**nnn_l * Dlc
              Db_eff(i,j,k) = (ne(i,j,k)-theta_th(i,j,k))**mmm_l * &
                   ((theta(i,j,k)-theta_th(i,j,k))/(ne(i,j,k)-theta_th(i,j,k)))**nnn_l * Db
              Dlo_eff(i,j,k) = (ne(i,j,k)-theta_th(i,j,k))**mmm_l * &
                   ((theta(i,j,k)-theta_th(i,j,k))/(ne(i,j,k)-theta_th(i,j,k)))**nnn_l * Dlo
              Dgo_eff(i,j,k) = ne(i,j,k)**mmm_g*((epsi(i,j,k)-epsi_th(i,j,k))/(ne(i,j,k)-epsi_th(i,j,k)))**nnn_g * Dgo
              Dld_eff(i,j,k) = (ne(i,j,k)-theta_th(i,j,k))**mmm_l * &
                   ((theta(i,j,k)-theta_th(i,j,k))/(ne(i,j,k)-theta_th(i,j,k)))**nnn_l * Dld
              Dgd_eff(i,j,k) = ne(i,j,k)**mmm_g*((epsi(i,j,k)-epsi_th(i,j,k))/(ne(i,j,k)-epsi_th(i,j,k)))**nnn_g * Dgd
           endif
        enddo
     enddo
  enddo
     ...
     do j=1,Ny
        do k=1,Nz  
           if(.not.wall(i,j,k)) then
              ...
              alpha_c(i,j,k) = rhos*(1.-ne(i,j,k))*k_m(i,j,k)/theta(i,j,k)*SA  !!! alpha_c=rhos*(1-ne)*k_m/theta*SA
           endif
        enddo
     enddo
  enddo
              ...
              if(wall(i,j,1)) then
                 ...
                 if(lcbc_d.eq.1) then 
                    aa(i,j,1) = theta(i,j,1) + dt*(-min(u(i,j,1),0.)*(theta(i-1,j,1)+theta(i,j,1))/2./dx + &
                         ...
                         dt*theta(i,j,1)*(-qc(i,j,1)*b_o(i,j,1)/(Kc+lc_o(i,j,1))*lo_o(i,j,1)/(Ko+lo_o(i,j,1))) + &
              ...
              if(wall(i,j,Nz)) then
                 ...
                 if(lcbc_t.eq.1) then
                    aa(i,j,Nz) = theta(i,j,Nz) + dt*(-min(u(i,j,Nz),0.)*(theta(i-1,j,Nz)+theta(i,j,Nz))/2./dx + &
                         ...
                         dt*theta(i,j,Nz)*(-qc(i,j,Nz)*b_o(i,j,Nz)/(kc+lc_o(i,j,Nz))*lo_o(i,j,Nz)/(ko+lo_o(i,j,Nz))) + &
              ...
              Do k=2,Nz-1
                 if(wall(i,j,k)) then
                    ...
                    aa(i,j,k) = theta(i,j,k) + dt*(-min(u(i,j,k),0.)*(theta(i-1,j,k)+theta(i,j,k))/2./dx + &
                         ...
                         (Dlc_eff(i,j,k+1)+Dlc_eff(i,j,k))/2./dz*(theta(i,j,k+1)+theta(i,j,k))/2./dz) - &
                         dt*theta(i,j,k)*(-qc(i,j,k)*b_o(i,j,k)/(Kc+lc_o(i,j,k))*lo_o(i,j,k)/(Ko+lo_o(i,j,k))) + &
        ...
        Do j=1,Ny
           Do i=1,Nx
              if(ne(i,j,k).ne.1.0 .and. .not.wall(i,j,k)) then
                 sc(i,j,k) = 1./(1.+dt*a1*theta(i,j,k)*Dlc/(theta(i,j,k)+b1)*SA*Ka) * &
                      (sc_o(i,j,k) + dt*a1*theta(i,j,k)*Dlc/(theta(i,j,k)+b1)*SA*lc_o(i,j,k))
              endif
           Enddo
        Enddo
     Enddo
        ...
        Do j=1,Ny
           Do k=1,Nz
              if(wall(i,j,k)) then
                 ...
                 b(i,j,k) = b_o(i,j,k) + &
                      dt*(Yd*qc(i,j,k)*b_o(i,j,k)*lc_o(i,j,k)/(Kc+lc_o(i,j,k))*lo_o(i,j,k)/(Ko+lo_o(i,j,k)) - Dk*b_o(i,j,k))
              endif
           Enddo
        Enddo
     Enddo
              ...
              if(wall(i,j,1)) then
                 ...
                 if(tobc_d.eq.1) then 
                    aa(i,j,1) = ne(i,j,1) + dt*(-(alpha1_o(i-1,j,1)+alpha1_o(i,j,1))/2.*min(u(i,j,1),0.)* &
                         ...
                         (alpha2_o(i,j,1+1)+alpha2_o(i,j,1))/2.*(Dgo_eff(i,j,1+1)+Dgo_eff(i,j,1))/2.)/dz* &
                         (ne(i,j,1+1)+ne(i,j,1))/2./dz) - &
                         dt*ne(i,j,1)*(-alpha1_o(i,j,1)*qo(i,j,1)*b_o(i,j,1)* &
              ...
              if(wall(i,j,Nz)) then
                 ...
                 if(tobc_t.eq.1) then
                    aa(i,j,Nz) = ne(i,j,Nz) + dt*(-(alpha1_o(i-1,j,Nz)+alpha1_o(i,j,Nz))/2.*min(u(i,j,Nz),0.)* &
                         ...
                         dt*ne(i,j,Nz)*(-alpha1_o(i,j,Nz)*qo(i,j,Nz)*b_o(i,j,Nz)* &
              ...
              Do k=2,Nz-1
                 if(wall(i,j,k)) then
                    ...
                    aa(i,j,k) = ne(i,j,k) + dt*(-(alpha1_o(i-1,j,k)+alpha1_o(i,j,k))/2.*min(u(i,j,k),0.)* &
                         ...
                         (alpha2_o(i,j,k+1)+alpha2_o(i,j,k))/2.*(Dgo_eff(i,j,k+1)+Dgo_eff(i,j,k))/2.)/dz* &
                         (ne(i,j,k+1)+ne(i,j,k))/2./dz) - &
                         dt*ne(i,j,k)*(-alpha1_o(i,j,k)*qo(i,j,k)*b_o(i,j,k)* &
              ...
              Do j=2,Ny-1
                    if(wall(i,j,Nz)) then
                       ...
                       aa(i,j,Nz) = ne(i,j,Nz) + dt*(-(alpha1_o(i-1,j,Nz)+alpha1_o(i,j,Nz))/2.*min(u(i,j,Nz),0.)* &
                            ...
                            dt*ne(i,j,Nz)* &
                            (-alpha1_o(i,j,Nz)*qo(i,j,Nz)*b_o(i,j,Nz)* &
                            lc_o(i,j,Nz)/(Kc+lc_o(i,j,Nz))/(Ko+lo_o(i,j,Nz)))
              ...
              if(wall(i,j,1)) then
                 ...
                 if(tdbc_d.eq.1) then 
                    ...
                    q(i,j,1) = ne(i,j,1)*td_o(i,j,1)  + &
                            dt*theta(i,j,1)* &
                            (qd(i,j,1)*b_o(i,j,1)*lc_o(i,j,1)/(Kc+lc_o(i,j,1))*lo_o(i,j,1)/(Ko+lo_o(i,j,1))) - &
              ...
              if(wall(i,j,Nz)) then
                 ...
                 if(tdbc_t.eq.1) then
                    ...
                    q(i,j,Nz) = ne(i,j,Nz)*td_o(i,j,Nz) + &
                         dt*theta(i,j,Nz)* &
                         (qd(i,j,Nz)*b_o(i,j,Nz)*lc_o(i,j,Nz)/(kc+lc_o(i,j,Nz))*lo_o(i,j,Nz)/(ko+lo_o(i,j,Nz))) - &
              ...
              Do k=2,Nz-1
                 if(wall(i,j,k)) then
                    ...
                    q(i,j,k) = ne(i,j,k)*td_o(i,j,k) + &
                         dt*theta(i,j,k)* &
                         (qd(i,j,k)*b_o(i,j,k)*lc_o(i,j,k)/(Kc+lc_o(i,j,k))*lo_o(i,j,k)/(Ko+lo_o(i,j,k)))
                 endif
              Enddo
           Enddo
        Enddo
           ...
           Do i=1,Nx
              Do j=1,Ny
                 if(tdbc_t.eq.1) then
                    ...
                    fld(i,j) = w(i,j,Nz+1)*ld_t + 2.*Dld_eff(i,j,Nz)*(ld(i,j,Nz)-ld_t)/dz
                    mld(i,j) = fld(i,j)*dt*theta(i,j,Nz)*dx*dy
                 endif
                 if(tdbc_t.eq.1) then
                    fgd(i,j) = 2.*Dgd_eff(i,j,Nz)*(gd(i,j,Nz)-gd_t)/dz 
                    mgd(i,j) = fgd(i,j)*dt*epsi(i,j,Nz)*dx*dy
                 endif
              Enddo
           Enddo
           tfld = sum(fld)
...
!!$  Write(*,*)'## Melocular diffusion coefficient of bacteria = ',Db
     ...
     Read(*,*),Ka
     Write(*,*)'## adsorption/desorption equilibrium constant Ka =',Ka
     Read(*,*),SA
...
!!$     Read(*,*),tau1
!!$     Write(*,*)'## Parameter tau1 for capillary pressure = ',tau1
!!$     Read(*,*),lambda1
!!$     Write(*,*)'## Parameter lambda1 for capillary pressure = ',lambda1
!!$     Read(*,*),Sm
!!$     Write(*,*)'## Parameter Sm = ',Sm

  Read(*,*),a_th
  Write(*,*)'## Parameter for theta_th = ',a_th
  Read(*,*),CF
  Write(*,*)'## clay fraction = ',CF
  Read(*,*),OM
  Write(*,*)'## Organic matter = ',OM
  Read(*,*),a_ep
  Write(*,*)'## Parameter for epsi_th = ',a_ep
  Read(*,*),Kp
  Write(*,*)'## half-saturatd osmotd stress = ',Kp
  Read(*,*),w_os
  Write(*,*)'## sensitive arameter for osmotd stress = ',w_os
...
!  CALL MPI_BCAST(gdbc_t,1,MPI_INTEGER,0,MPI_COMM_WORLD,error)
  ...
  CALL MPI_BCAST(Ka,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
...
!!$  CALL MPI_BCAST(Sm,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  ...
  CALL MPI_BCAST(a_th,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  CALL MPI_BCAST(CF,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  CALL MPI_BCAST(OM,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  CALL MPI_BCAST(a_ep,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  CALL MPI_BCAST(Kp,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
  CALL MPI_BCAST(w_os,1,MPI_double_precision,0,MPI_COMM_WORLD,error)
...
SUBROUTINE WriteMatlab()
  ...
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'b = zeros( ',Max_step/File_step+1,&
       ',', LX,',', LY,',', LZ,');'
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'o = zeros( ',Max_step/File_step+1,&
       ',', LX,',', LY,',', LZ,');'
  ...
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'go = zeros( ',Max_step/File_step+1,&
       ',', LX,',', LY,',', LZ,');'
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'d = zeros( ',Max_step/File_step+1,&
       ',', LX,',', LY,',', LZ,');'
  ...
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'b(', 1,',:,:,:) = ReadReal( ',&
       LX,',', LY,',', LZ,',''', trim(Rootname),''',''float64'');'
  Rootname = '../'//trim(IN_DIR)//'/'//trim(PMname)//'_o'
  ...
  WRITE(20,'(A,I8,A,I8,A,I8,A,I8,A,A,A)')'go(', 1,',:,:,:) = ReadReal( ',&
       LX,',', LY,',', LZ,',''', trim(Rootname),''',''float64'');'
  Rootname = '../'//trim(IN_DIR)//'/'//trim(PMname)//'_d'
